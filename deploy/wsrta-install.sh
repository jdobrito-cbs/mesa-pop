...
# banco/usuario derivados do dominio (evita colidir com apps/roles antigos).
...
if [ -f .env ]; then
  ...
  DB_URL="$DATABASE_URL"       # banco gerenciado externamente pelo painel
else
  DB_URL="postgresql://mp_${SLUG}:$(rand 16)@localhost:5432/mp_${SLUG}"
fi

# ---- Postgres LOCAL: garante role, SENHA (forcada) e banco (idempotente) ----
case "$DB_URL" in
  *@localhost:*|*@127.0.0.1:*)
    command -v psql >/dev/null 2>&1 || die "PostgreSQL (psql) nao encontrado. Instale ou informe DATABASE_URL externo."
    ...
    PSQL -c "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname='${DB_USER}') THEN CREATE ROLE \"${DB_USER}\" LOGIN; END IF; END \$\$;" \
      || die "Sem permissao para criar o usuario do banco. Informe um DATABASE_URL externo."
...
if command -v psql >/dev/null 2>&1; then
  psql "$DB_URL" -tAc "SELECT 1" >/dev/null 2>&1 \
    || die "Nao consegui conectar no banco com as credenciais geradas. Confira o PostgreSQL do servidor (pg_hba/porta) ou informe DATABASE_URL."
fi
